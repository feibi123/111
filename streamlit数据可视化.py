...
uploaded_file2 = st.sidebar.file_uploader("上传发货表", type="txt")
...
dc = pd.read_csv(uploaded_file1, header=0)
...
df['是否发货'] = df['最晚发货时间'].apply(fill_or_not)


df['建议补货数量'] = df['平均销量'] * (variable5 - variable4 - df['总预计可售天数'])  # 计算建议补货数量
df.loc[(df['平均销量'] == 0) | (df['总预计可售天数'] == 0), '建议补货数量'] = 0
