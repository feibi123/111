...
variable = 31  # 可变天数
variable2 = 30  # 30天安全库存
variable3 = 60  # 60天安全库存
variable4 = 7  # 最小安全库存
variable5 = 45  # 物流周期
...
#  df7['quantity'] = df7['quantity'] / 7  # 计算7天内日平均销量
#  day7 = df7[['sku','quantity']]  # 整合放入day7表中
...
#  df15['quantity'] = df15['quantity'] / 15  # 计算15天内日平均销量
#  day15 = df15[['sku','quantity']]  # 整合放入day15表中
...
#  dfv['quantity'] = dfv['quantity'] / variable  # 计算变量天数的日平均销量
#  dayvariable  = dfv[['sku','quantity']]
...
dc = pd.read_excel(r'E:\发货表\FSM\产品属性表-FSM.xlsx', header=0)  # 读取产品属性表，并将首行作为标题列
...
df[['在库预计可售天数', '总预计可售天数', '安全库存']] = df[['在库预计可售天数', '总预计可售天数', '安全库存']].round().astype(int)  # 保留整数
...
with pd.ExcelWriter(r'E:\发货表\FSM\发货表.xlsx') as writer:  # 保存文件
    ...
    styler = df.style.applymap(style_cell, subset=['在库预计可售天数', '总预计可售天数'])
    formatted_styler = styler.format("{:.0f}")
    st.write(formatted_styler)
